...
- 查询优先读取流量响应头，SIP008 回退正文限制为 2 MiB；仅同一 HTTPS 主机和端口可重定向。
- 仅临时连接错误切换本机代理路径；取消请求停止重试，删除订阅取消在途请求，自动失败刷新退避，手动刷新不受退避限制。
- 同步五种语言的安全说明与新增错误提示。55 项自动测试、签名候选包的 11 项真实钥匙串检查通过；使用独立模拟订阅，未进行真实账户迁移或 GitHub 线上 OTA 安装联测。Developer ID 签名，尚未 Apple 公证。
