...
システムの TLS 証明書検証を維持し、HTTP へのダウングレードを拒否します。トークンを含む可能性のある生のネットワークエラーは画面に表示しません。手動追加した URL はローカル設定に保存され、**キーチェーンによる暗号化保存ではありません**。端末の侵害、ローカルデータへのアクセス、バックアップの漏えいに対する安全性を保証するものではありません。
