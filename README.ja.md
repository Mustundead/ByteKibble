...
> **現在はドキュメントとアイコンのみの更新です。** このページは調整中の新バージョンを紹介しています。対応するアプリコード、初回起動時のウェルカム画面、ZIP／DMG パッケージは今回公開していません。古いコードや `preview/` 内の過去の画像は、ここで紹介する新デザインとは異なります。ダウンロードは [Releases](https://github.com/mustundead/ByteKibble/releases) で実際に公開されたバージョンをご確認ください。
...
- **必要なときに警告**：残量が 20% 未満ならオレンジ、7% 未満なら赤。残量不足や使い切った状態は文字でも示します。
...
| --- | --- | --- |
| 20% 以上 | 外観に応じた黒／白 | 通常 |
| 7% 以上、20% 未満 | オレンジ | 残量不足 |
| 7% 未満 | 赤 | 残量がごくわずか、または使い切った状態 |
...
ドラッグでインストールできる ZIP／DMG と 1 ページのウェルカム画面は準備中です。**今回のドキュメント更新に新しいインストーラは含まれません**。公開後は本リポジトリの [Releases](https://github.com/mustundead/ByteKibble/releases) から入手できます。ZIP は展開したアプリを「アプリケーション」へ移動し、DMG は開いてアプリを「アプリケーション」のショートカットへドラッグします。移動後のアプリを「アプリケーション」から起動してください。
...
サードパーティーの実行時依存関係はありません。ビルド成功は、署名・公証・インストーラの検証が済んだことを意味しません。ドキュメントを先行更新しているため、実際の動作はチェックアウトしたコードに基づきます。
...
**過去の許諾は変更しません。** 以前 MIT で提供したコードとバージョン（1.2.0（41）の確認用パッケージを含む）には、商用利用の権利を含めて[従来の MIT 条件](LICENSES/MIT-legacy.txt)が引き続き適用されます。今回はライセンス文書と README のみを更新し、新しいアプリコードやインストーラはアップロードしません。
